...
describe RakeEasyRSA::Tasks::Client::Revoke do
  ...
  it 'adds a revoke task in the namespace in which it is created' do
    ...
    expect(Rake::Task.task_defined?('client:revoke'))
        .to(be(true))
  end
  ...
  it 'gives the revoke task a description' do
    ...
    expect(Rake::Task['client:revoke'].full_comment)
        .to(eq('Revoke a client certificate for the PKI'))
  end

  it 'uses the underlying default PKI directory by default' do
    define_task

    rake_task = Rake::Task['client:revoke']
  ...
  it 'revokes a client certificate' do
    ...
    Rake::Task['client:revoke'].invoke(filename_base)
  end
